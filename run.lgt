...
:- object(report_generator).
    ...
    start :-
        write('Enter the name of the CSV file: '),
        readutil:read_line_to_string(user_input, FileName),
        (   exists_file(FileName)
        ->  write('Reading employee data from file...'), nl,
            load_employee_data(FileName, Employees),
        ...
        ;   write('Error: The file does not exist.'), nl
        ).
    ...
    generate_report(Employees, File) :-
        ...
        forall(
            lists:member(employee(Name, _, Position), Employees),
            (
                % Логотип
